...
7. Fill dpp_fields (material, recyclability class in plain words, est_recycled_content_pct) as draft Digital Product Passport data.

Be specific and concise. Never invent a brand you cannot see. Always return valid data for every field.`;

export function buildScanUserText(municipalityBlock: string): string {
  return `Municipality ruleset (apply to local_hint):
${municipalityBlock}
